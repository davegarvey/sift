...
- **One pairing modal** — `SyncJoinModal` + `SyncShareModal` merge into a single `PairDeviceModal` with both halves visible (no device-type detection, per the spec): source half shows the OTP code + QR with expiry ring; target half accepts an 8-character code **or** a 22-character sync key (paste flow, spec-mandated, previously absent) with Enter-to-submit, plus the existing camera scan path. The `/?pair=` deep-link flow is untouched.
