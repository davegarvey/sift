...
- [x] 7.5 Implement the keyboard navigation: `j`/`k` to move focus, `Enter` to open, `Esc` to return
  - **Post-implementation fix**: `scrollIntoView({ block: 'nearest' })` on `focusedIndex` change existed in `River.tsx` but was wired to `onMount` instead of `createEffect`. Changed to `createEffect` so the river scrolls to keep the focused item in view on every `j`/`k` navigation, matching the spec at `reader-ui/spec.md:61`.
