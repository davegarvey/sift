...
- **Delayed loading message in the river**: When a feed (or the "All" view) has zero items because the initial fetch is still in progress (or the app is hydrating from IndexedDB on boot), show a simple "Loading…" message that fades in only after ~500ms instead of the "No items yet" empty state — fast loads never see it
...
- `reader-ui`: When the river has no items because a feed is being fetched for the first time (or after the user triggered a refresh), or while the app hydrates from IndexedDB on boot, show a "Loading…" message that fades in after ~500ms instead of the "No items yet" empty state. The sidebar shows a per-feed spinner while a feed fetch is in flight. After a subscribe's fetch resolves, items appear immediately without waiting for the periodic polling interval.

## Impact

- `src/feeds/scheduler.ts` — add a per-feed fetching state signal so the UI can react to individual feed fetch progress
- `src/components/River.tsx` — render a delayed, fading "Loading…" message when the empty state is due to an in-flight fetch or startup hydration
...
- `src/styles.css` — add `.loading-message` fade-in styles and sidebar spinner animation
- No new dependencies
