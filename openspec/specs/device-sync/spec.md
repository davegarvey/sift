...
#### Scenario: Matching feed URLs adopt the server identity
- **WHEN** a local feed URL matches a remote feed URL but their feed IDs differ
- **THEN** the client SHALL re-key the local feed and all feed-linked local records to the remote `feed_id` before applying flags
- **AND** the client SHALL preserve current read/starred state, read markers, and feed statistics
- **AND** subsequent flag pushes SHALL use the canonical remote feed ID directly
