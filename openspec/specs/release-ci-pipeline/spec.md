...
### Requirement: Non-code changes skip Docker publish

The system SHALL skip Docker publish when only documentation, configuration, or meta files are changed. CI SHALL run on all pull requests, including documentation-only ones (a `paths-ignore` on the PR trigger was tried and reverted because docs-only PRs then never reported the required `test` check, blocking branch protection).

#### Scenario: Documentation-only PR runs CI
